...
int main(int argc, char **argv)
{
	...
	*/
	...
	for (;;)  /* for debugging force to disable enable loop forever */
	{
	/* change the number of layers to just 2 */
	/* disable each layer and compose as we go so that you can see it */
	//for (i=LAYER_START; i <= LAYER_END;  i++)
	for (i=LAYER_START; i <= 2;  i++)
	{
	...
	/* clear each layer and compose as we go so that you can see it */
	for (i=LAYER_START; i <= LAYER_END;  i++)
	{
		ret = Rd_LayerClear(rd_interface, i); 
		if (ret != STATUS_OK) return ret;
	...
	/* enable layer, load image and write it 
	do this for every ODD element of the list
	stop when element has image_layer = ENDLIST
	*/
	for (i=0; image_list[i].image_layer != ENDLIST; i++)
	{
	...
	/* enable layer, load image and write it 
	do this for every EVEN element of the list
	stop when element has image_layer = ENDLIST
	*/
	for (i=0; image_list[i].image_layer != ENDLIST; i++)
	{
